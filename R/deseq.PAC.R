...
#'
#' \code{deseq.PAC} DESeq2 analysis on PAC_object.
#'
...
#'          
#'          Note: In deseq.PAC pheno_target controls the main comparative factor
...
#' 
#' test <- deseq(pac, model=model, deseq_norm=TRUE, histogram=TRUE, threads=4, pheno_target=pheno_target, anno_target=NULL)
#' 
#' #export
# deseq <- function(x, ...) UseMethod("deseq", x)
# deseq<- function(x) {
# UseMethod("deseq")
# }
# deseq.PAC <- function(PAC, model, deseq_norm=FALSE, histogram=TRUE, threads=1, pheno_target=NULL, anno_target=NULL){
#   
