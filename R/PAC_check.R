...
PAC_check <- function(PAC, sample_check=NULL, seq_check=NULL){
  sampl <- paste0("\nPlease make sure that all samples are represented,",
                  "\nnamed and ordered the correct way in both tables.") 
  seqs <-  paste0("\nPlease make sure that all sequences are represented,", 
                  "\nnamed and ordered the correct way in both tables.")
  if(!identical(rownames(PAC$Pheno), colnames(PAC$Counts))){
    stop(paste0(
      "\n  Sample names in Pheno (row names) and Counts (column names)", 
      "\n  are not identical.\n  ", sampl))
  }
  if(!identical(rownames(PAC$Anno), rownames(PAC$Counts))){
    stop(paste0(
      "\n  Sequence names in Anno (row names) and Counts (row names)", 
      "\n  are not identical.\n  ", seqs))
  ...
  if(length(PAC$norm) > 0){ 
    ...
    if(norm_logi_row){
      stop("\nRow names in normalized tables of PAC$norm are ",
           "\nnot identical with row names in Counts.")
    }
    if(norm_logi_col){
      stop("\nColumn names in normalized tables of PAC$norm are",
           "\nnot identical with column names in Counts.")
    }
  }
  if(length(PAC$summary) > 0){ 
    ...
    if(sum_logi_row){
      stop("\nRow names in summary tables of PAC$summary are ",
           "\nnot identical with row names in Counts.")
    }
  } 
  res <- NULL
      ...
      if(is.character(sample_check)){
        if(length(sample_check)==1){
          whr  <- which(unlist(lapply(PAC, function(x){  
            ...
            if(class(x)=="list"){
              any(grepl(sample_check, names(x)))
            }
          })))
          if(length(whr)==1){
            if(sum(grepl(paste(rownames(PAC$Pheno)[1:2], collapse="|"), 
              ...
              if(sum(grepl(paste(rownames(PAC$Pheno)[1:2], 
                                 collapse="|"), 
                           colnames(PAC[[whr]][[sample_check]])))==2){
                if(!identical(rownames(PAC$Pheno), 
                              colnames(PAC[[whr]][[sample_check]]))){
                  stop(
                    paste0("\n  Sample (column) names in new table are not ",
                           "identical",
                           "\n  with sample names in the main PAC.\n  ", sampl))
                  }
              }else{   
                stop("\nPAC_check did find a matching object name, but could ",
                     "\nnot find the samples. If you ment to check sequences ",
          ...
          if(length(sample_check)>1){ 
            if(!identical(rownames(PAC$Pheno), sample_check)){
              stop(paste0("\n  Sample names are not identical with sample ",
                          "\n  names in the main PAC.\n  ", sampl))
            }
          }
        }
        ...
        stop("\nPAC_check did not recognize the sample_check input.",
             "\nDid you provide a data.frame, character vector or ",
             "\nthe name of a PAC object?")
        }  
    }}
  ### seq_check #############################################################          
