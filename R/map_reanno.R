...
#' 
#' map_reanno(pac, input=input, output=output,
...
#' #    
#' # input <- list(genome="<path_to_bowtie_indexed_fasta>")
...
#'                            }
#'                            
#'  input <- list(genome1=mycoplasma_file, genome2=mycoplasma_file)
#'  map_reanno(PAC=pac, input=input, output=output, 
...
#' @export
# 
map_reanno <- function(PAC, type="internal", output, input, 
  ...
  input <- lapply(input, function(x){gsub("\\.fa$", "", x)})
  ...
  for(i in seq.int(length(mis_lst))){
    ...
    for (j in seq.int(length(input))){
      cat("\n   |--->", paste0(names(input)[j], "..."))
      out_fl_nam <- paste0(names(input)[j], ".out")
      ...
      if(type=="external"){
        ...
        bwt_exp <- paste0("bowtie ", parse_external, 
                          " -v ", mis_lst[[i]], " -p ", threads, " ", 
                          input[[j]], " ", input_file, " ", output_file)
