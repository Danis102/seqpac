...
#' #                          summary_target=list("means_[Method]"), 
#' #                          xseq=TRUE, style="line", colour=c("red", "black"))                                   
#' #  
...
PAC_mismap <- function(PAC, map, type="percent", summary_target=NULL, 
                       map_target=NULL,  threads=1, par_type="PSOCK", 
                       xseq=TRUE, colors=NULL){

 j <- position <- position <- value <- variable <- NULL
  
  if(is.null(summary_target[[1]])){
    stop(
      "\nYou need to specify a list in PAC$summary using summary_target.")
    }
  ...
  for(i in 1:length(map)){
    ...
    res_lst <- lapply(start_lst, function(x){
      seq_lst <- split(map[[i]][[2]], 
                       factor(row.names(map[[i]][[2]]), 
                              levels=row.names(map[[i]][[2]])))
      res <- data.frame(position=1:nchar(paste0(map[[i]][[1]])), 
                        ref_nt=unlist(strsplit(paste0(map[[i]][[1]]), "")), 
                        A=0, T=0, G=0, C=0)
      PAC=PAC
      summary_target=summary_target
      doParallel::registerDoParallel(threads) 
      `%dopar%` <- foreach::`%dopar%`
      positions_lst <- foreach::foreach(
        j=1:length(seq_lst), .final=function(y){
        names(y) <- names(seq_lst);return(y)})  %dopar% {
            res_work  <- res
            seqs_pos <- data.frame(
              position=seq_lst[[j]]$Align_start:seq_lst[[j]]$Align_end, 
              nt=unlist(strsplit(paste0(rownames(seq_lst[[j]])), "")))
            res_work$hits[
              res_work$position %in% seqs_pos$position] <- as.character(
                seqs_pos$nt)
            res_work$hits[is.na(res_work$hits)] <- "No_hit"
            data  <- PAC$summary[[
              summary_target[[1]]]][rownames(
                PAC$summary[[summary_target[[1]]]]) %in% rownames(
                  seq_lst[[j]]),,drop=FALSE]
            
            res_work$A[res_work$hits=="A"] <- res_work$A[
              res_work$hits=="A"]+ data[, colnames(data) == x]
            res_work$T[res_work$hits=="T"] <- res_work$T[
              res_work$hits=="T"]+ data[, colnames(data) == x]
            res_work$C[res_work$hits=="C"] <- res_work$C[
              res_work$hits=="C"]+ data[, colnames(data) == x]
            res_work$G[res_work$hits=="G"] <- res_work$G[
              res_work$hits=="G"]+ data[, colnames(data) == x]
            return(res_work[, !colnames(res_work)=="hits"])
      }
      stopifnot(!any(!do.call("c",lapply(positions_lst, function(v){
        identical(v$ref_nt, positions_lst[[1]]$ref_nt)
        }))))
      stopifnot(!any(!do.call("c",lapply(positions_lst, function(v){
        identical(v$position, positions_lst[[1]]$position)
        }))))
      stopifnot(identical(names(positions_lst), rownames(map[[i]]$Alignments)))
      A_vec <- rowSums(do.call("cbind", lapply(positions_lst, function(z){
        return(z[, "A", drop=FALSE])})))
      T_vec <- rowSums(do.call("cbind", lapply(positions_lst, function(z){
        return(z[, "T", drop=FALSE])})))
      C_vec <- rowSums(do.call("cbind", lapply(positions_lst, function(z){
        return(z[, "C", drop=FALSE])})))
      G_vec <- rowSums(do.call("cbind", lapply(positions_lst, function(z){
        return(z[, "G", drop=FALSE])})))
      fin_df <- data.frame(position=positions_lst[[1]]$position, 
                           ref_nt=positions_lst[[1]]$ref_nt, 
                           A=A_vec, T=T_vec, C=C_vec, G=G_vec)
      return(fin_df)
    })
    fin_lst[[i]] <- res_lst
  ...
  #### Plot data ####
  if(is.null(colors)){
    colors <- c(A="#094A6B", T="#e6a700",
                C="#9D0014", G="#52793d", ref="#B9B9B9")
    #scales::show_col(colors)
  }
  
  if(type=="percent"){
    plt_data <- perc_lst; ylb <- "Mean percent"
    }
  if(type=="rpm"){
    plt_data <- fin_lst; ylb <- "Mean rpm"
    }
  
  plot_lst <- lapply(seq_along(plt_data), function(z){ 
    lst <- plt_data[[z]]
    ...
    }else{x_lab<-NULL; tcks <- ggplot2::element_blank()}
    ...
    plts <- lapply(seq_along(lst), function(q){ # Use seq_along to grab names
      ...
      data_long$position <- factor(data_long$position, 
                                   levels=unique(data_long$position))
      
      p <-  ggplot2::ggplot(
        data_long, ggplot2::aes(x=position, y=value, fill=variable)) +
        ggplot2::geom_bar(stat="identity", width=0.9, size=1.0) +
        #geom_hline(yintercept=0, col="#707177", cex=0.6) +
        ggplot2::scale_fill_manual(values=rev(colors))+
        ggplot2::ylab(ylb) +
        ggplot2::ggtitle(nam2) +
        ggplot2::xlab(nam1) +
        ggplot2::scale_x_discrete(labels=c(as.character(df$ref_nt)))+
