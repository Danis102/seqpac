...
#' element they silence. Silincing targets may differ from cell-type to
...
#' 
#' #######################################################################
#' ### Convert of UCSC to Ensembl  
#' 
#' # There are a few places where conversion tables between databases like 
#' # (e.g. UCSC <-> Ensembl <-> refSeq) are available. 
#' # Devon Ryan and co-workers provide  such at Github: 
#' # https://github.com/dpryan79/ChromosomeMappings
#' # Some conversion tables are also available from the UCSC ftp:
#' # ftp://hgdownload.cse.ucsc.edu/goldenPath
#' # For drosophila melanogaster dm6:
#' # ftp://hgdownload.cse.ucsc.edu/goldenPath/dm6/bigZips/dm6.chromAlias.txt  
#' 
#' # Sometimes a conversion table is missing. Seqpac can generate conversion
#' # tables from 2-3 references fasta containing identical sequences (e.g. 
#' # reference genomes from the same release) using the make_conv function. This
#' # function also guarantees that you are using the same relaese in your genome
#' # files.
#' 
...
# 
# 
# PAC_pirna <- function(PAC, 
#                       
#                       
#                       
#                       pirna=NULL, pirna_meta=NULL,  genome=NULL, 
#                       chrom_size=NULL, mismatches=3, stranded=FALSE,
#                        gtf_repeat=NULL, gtf_protein=NULL, norm="counts", 
#                       pheno_target=NULL, threads=1){
# 
#   
#   
# 
...
#    }
#    
#    anno <- quiet(PAC_nbias(PAC, data_only=TRUE))
#    anno$'10_nuc_bias' <- (quiet(PAC_nbias(PAC, data_only=TRUE, position = 10)))[,2]
#    anno <- PAC$Anno  
#   
#   ### HÄR ÄR JAG Fortsätt att samla kolumner
#   
#   
#   
#   
...
#   
# 
# PAC_pirna <- function(PAC, pirna=NULL, pirna_meta=NULL,  genome=NULL, 
#                       chrom_size=NULL, mismatches=3, stranded=FALSE,
#                        gtf_repeat=NULL, gtf_protein=NULL, norm="counts", 
#                       pheno_target=NULL, threads=1){  
#   
...
#        PAC <- PAC_norm(PAC, type=norm, merge_pac=TRUE)
#        cat("\n")
...
#      }
#    }
#   PAC <- PAC_summary(PAC, norm=norm, type="means", pheno_target=pheno_target, merge_pac = TRUE)
#   sum_nam_pheno  <- paste0(norm,"Means_", pheno_target[[1]])
# 
#   PAC <- suppressWarnings(PAC_summary(PAC, norm=norm, type="means", pheno_target=NULL, merge_pac = TRUE))
#   sum_nam_all  <- paste0(norm,"Means_All")
# 
#   PAC <- suppressWarnings(PAC_summary(PAC, norm=norm, type="log2FC", pheno_target=pheno_target, merge_pac = TRUE))
