...
#'  
#'@param type Character indicating what type of 
#'
...
#'  biotype(s) in the target column (1st object). (default=NULL)
#'
...
#'  group(s) in the target column (1st object). (default=NULL)
#'
#'@param labels If labels="target", individual points in the scatter plots will
#'  be labeled with the enteries of the pheno_target or anno_target columns. If
#'  labels="sample", then point will be labeled with the names rownames in
#'  PAC$Pheno or PAC$Anno depending on type="pheno" or type="anno",
...
#' 
#' # Same but with sample name text labels: 
#' pca_rpm_lab <- PAC_pca(pac_master, norm="rpm", type="pheno", pheno_target=list("type"), labels="text")
#' 
#' # Same but with labels provided as a custom vector: 
...
PAC_pca <- function(PAC, norm="counts", type="pheno", graphs=TRUE, pheno_target=NULL, anno_target=NULL, labels=NULL){
  ...
  if(!is.null(labels)){
      geom <- c("point", "text")
      if(length(labels) > 1){
            if(any(duplicated(labels))){colnames(data) <- unlist(lapply(split(labels, as.factor(labels)), function(x){paste(x, 1:length(x), sep="_")}))
  ...
  if(type=="both"){
    grphs$PC1_PC2 <- factoextra::fviz_pca_biplot(pca_res, habillage = col, geom=c("arrow", "text"), geom.var = "point", col.var = "black", repel=TRUE, addEllipses = FALSE, axes=c(1,2), invisible="quali", pointsize=1, labelsize=3, title="PC1_PC2 - Biplot")
    grphs$PC1_PC3 <- factoextra::fviz_pca_biplot(pca_res, habillage = col, geom=c("arrow", "text"), geom.var = "point", col.var = "black", repel=TRUE, addEllipses = FALSE, axes=c(1,3), invisible="quali", pointsize=1, labelsize=3, title="PC1_PC3 - Biplot")
    grphs$PC2_PC3 <- factoextra::fviz_pca_biplot(pca_res, habillage = col, geom=c("arrow", "text"), geom.var = "point", col.var = "black", repel=TRUE, addEllipses = FALSE, axes=c(2,3), invisible="quali", pointsize=1, labelsize=3, title="PC2_PC3 - Biplot")
