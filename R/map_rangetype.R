...
#'   (generated by \code{\link{map_reanno}}), containing mapping coordinates against a
...
#' sequences starting within 45-50% of the references as "type_mid_start_per"
...
#'   \code{\link{lapply}} prior to running the function.
#'   
...
#'   arguments in the \code{\link{PAC_mapper}} function. If N_include=FALSE (default),
#'   start and end of tRNA will be measured from the first and last canonical
          ...
          for(i in 1:length(map)){
              ...
              if(!is.null(N_counts[[i]])){
                  if(!align$n_hits[1] == "no_hits"){
                    ...
                    substring(align$Align_string, (nchar(align$Align_string) - N_counts[[i]][2] + 1)) <- paste(rep("*", times=N_counts[[i]][2]), collapse="")
                  }
              }
