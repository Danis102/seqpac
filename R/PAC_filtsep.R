...
#'   names and a Counts table with raw counts or normalized object containing
#'   for example reads per million (rpm; generated by \code{PAC_rpm}).
#'   
#'   PAC, norm="raw", threshold=10, coverage=100, pheno_target=NULL
#'   
#'   
...
#'   group that need to reach the threshold for a sequence to be included
#'   (default=100).
#'   
#' @param type Character specifying if filtering should be done using "counts",
#'   "rpm" or another normalized data table in PAC$norm (default="counts").
#'   
...
#' @export
#' 
PAC_filtsep <- function(PAC, type="counts", threshold=10, coverage=100, pheno_target=NULL){
  
                              ### Extract data ###
                                    if(type=="counts"){
                                            data <- PAC$Counts
                                    }else{  
                                          if(is.null(PAC$norm[[type]])){stop(paste0("There is no object called '", type, "' in the norm list.\n  (Hint: Did you forget to normalize the data using for example PAC_rpm,\n  or would you rather run the function on raw counts using norm='raw'?)"))}  
                                          data <- PAC$norm[[type]]
                                    }
