...
#'
#' @param input A list object containing imported data.frames from Sports output
#'   generated by \code{import_sports}, or a count table generated by
#'   \code{c(make.counts, make.counts2)}.
#'   
#' @param type Character indicating whether the annotation should be generated
...
#'   \code{\link[parallel]{detectcores}} to see available threads on the
#'   computer. Do not exceed the number of available threads!
#'
#'
#' @examples
...
make_anno_sports <- function(input, type="counts", threads=1, stat=TRUE){
              cat("\n")
              warning("This function is no longer maintained, and should be depricated.")      
              if(type=="counts"){
              ...
              if(type=="sports"){      
                        ...
                        seq_lst[[i]] <- as.character(input[[i]]$Sequence)
                        Sys.sleep(0.001)
                        ...
                        cat("\nOBS! Only RNA sequences that have at least evidence in 2 samples will be retained.\n\n")
                        cat("Now generating annotation table across all output files ...\n")
                        filt_seqs <- uni_seq[[1]]
                        rm(seq)
                        gc(reset=TRUE)
                        cat("Processing raw anno file 1 /", length(input), "at", paste0(Sys.time()), "\n")
                        filt_lst <- list(NA)
                        filt_lst[[1]] <- as.character(input[[1]]$Sequence) %in% filt_seqs
                        remains <- filt_seqs[!filt_seqs %in% as.character(input[[1]]$Sequence)]
                        for(i in 2:length(input)){
                                  filt_lst[[i]] <- as.character(input[[i]]$Sequence) %in% remains
                                  remains <- remains[!remains %in% as.character(input[[i]]$Sequence)]
                                  Sys.sleep(0.001)
                                  cat("Processing raw anno file", i, "/", length(input), "at", paste0(Sys.time()), "\n")
                        }
                        cat("Extracting sub tables at ", paste0(Sys.time()), "\n")
                        anno_lst <- list(NA)
                        pb <- txtProgressBar(min = 0, max = length(input), initial = 0, style = 3, width = 100)
                        for(i in 1:length(input)){
                                  anno_lst[[i]]<- input[[i]][filt_lst[[i]],]
                                  Sys.sleep(0.001)
                                  setTxtProgressBar(pb,i)
                        }
